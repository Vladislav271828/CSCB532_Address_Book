...
//    }
    ...
    public void importContactsFromJSON(String json) throws JsonProcessingException {
        checkIfUserIsAdmin();
        ...
        List<Contact> contactsToImport = importedContacts.stream()
                .map(dtoContact -> modelMapper.map(dtoContact, Contact.class))
                .toList();

        contactRepository.saveAll(contactsToImport);
    }
